...
export const getStaticProps = async ({ locale = defaultLocale }) => {
  const [page, meta, menu] = await Promise.all([
    PageService.pageGet("terms-of-use-page", {locale, ecommerceStoreId}, {apiPath, asValue: true}),
  ...
  const seo = SeoService.seoGet(meta, {key: "page_terms_of_use", page, websiteDomain});
