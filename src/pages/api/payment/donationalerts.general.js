const ERRORS = {
  noAccessToken:
    "AlertsAPI: No access token specified in config object in contructor",
  versionVariableType:
    "AlertsAPI: Version number specified in config object in contstructor must be number",
  alertInstanceIncorrect:
    "AlertsAPI: Provided alert to sendCustomAlert must be an instance of CustomAlert class",
};
...
class AlertsAPI {
  ...
  constructor(config) {
    ...
    if (config.access_token === undefined || config.access_token === "") {
      throw ERRORS.noAccessToken;
    }
    if (config.version !== undefined && typeof config.version !== "Number") {
      throw ERRORS.versionVariableType;
    }
    this.basePath = `https://www.donationalerts.com/api/v${
  ...
  static generateOauthLink(config) {
    ...
    return `https://www.donationalerts.com/oauth/authorize?client_id=${
      config.clientID
    }&redirect_uri=${encodeURI(
      config.redirectURI
    )}&response_type=code&scope=${encodeURI(scopes)}`;
  }
  ...
  async getDonations(page = 1, raw = false) {
    ...
    let request = await fetch(
      `${this.basePath}/alerts/donations?page=${page}`,
      {
        method: "GET",
        headers: { Authorization: `Bearer ${this.access_token}` },
      }
  ...
  async searchWithinMessage(term, pageLimit = 0, caseSensetive = true) {
    ...
    for (let page = 1; page <= pageLimit; page++) {
      ...
      donationsFiltered.push(
        ...donations.data.filter((donation) => {
          if (!caseSensetive) {
...
export default async (req, res) => {
  ...
  let token = await AlertsAPI.getAccessToken({
    clientID: 10231,
    clientSecret: 'RHXceS5Vb8D8iCq81X3p',
