...
import { MyCourseView } from "views/MyCourse";
...
export const getStaticProps = async ({ params, locale = defaultLocale }) => {
  ...
  const courseReportList = await CourseStructureService.courseReportList(courseId, {apiPath, asValue: true})
  ...
  return {
    ...
    props: {
      product,
      courseId,
      courseStructureList: courseStructureList.map(v => ({...v, is_preview: true})),
      courseReportList: courseReportList.map(v => ({...v, is_preview: true})),
